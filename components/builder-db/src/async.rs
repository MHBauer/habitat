...
impl AsyncServer {
    ...
    pub fn schedule(&self, dispatch_key: &str) -> Result<()> {
        ...
        let is_scheduled = {
            let mut r = self.retry.read().expect("Async retry lock is poisoned");
    ...
    fn get_num_events(&self, available_jobs: usize) -> Vec<(DispatchKey, EventFunction)> {
        ...
        r.iter()
            .filter(|&(dispatch_key, &(event_time, callback))| SteadyTime::now() >= event_time)
            .take(available_jobs)
            .map(|(dispatch_key, &(event_time, callback))| {
    ...
    fn retry_failed_event(&self, key: DispatchKey, event: EventFunction) {
        ...
        error!("Backing off {:?} for {:?}", key, next_event);

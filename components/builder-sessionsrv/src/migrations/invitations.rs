                   ...
                   oi_invite_id bigint, oi_ignore bool
                    ...
                    BEGIN
                            IF oi_ignore = true THEN
                                UPDATE account_invitations SET ignored = true, updated_at = now() WHERE id = oi_invite_id;
                            ELSE
                                SELECT origin_id, origin_name, account_id, account_name INTO oi_origin_id, oi_origin_name, oi_account_id, oi_account_name FROM account_invitations WHERE id = oi_invite_id;
                                PERFORM insert_account_member_v1(oi_origin_id, oi_origin_name, oi_account_id, oi_account_name);
                                DELETE FROM account_invitations WHERE id = oi_invite_id;
                            END IF;
                    END
                    $$ LANGUAGE plpgsql VOLATILE"#)?;
